...
contract CampaignFactory {
    ...
    string[] public deployedCampaignNames;

    function createCampaign(string memory name, uint256 minimum) public {
        address contractAddress = address(new Campaign(name, minimum, msg.sender));
        deployedCampaignNames.push(name);
        deployedCampaignContracts.push(contractAddress);
    }

    function getDeployedNames() public view returns (string[] memory) {
        return deployedCampaignNames;
    }
...
contract Campaign {
    ...
    constructor(string memory name, uint256 _minimum, address _owner) public {
        campaignName = name;
        manager = _owner;
